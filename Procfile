...
# Reduced max-tasks-per-child from 50→25 for faster memory recycling
...
celery-worker: celery -A core worker -l info --pool=prefork -c 1 --max-tasks-per-child=25 --max-memory-per-child=200000 -Q default,agents,sports
