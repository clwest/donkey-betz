...
export function Select({ value, onValueChange, children }: SelectProps) {
  ...
  return (
    <div ref={selectRef} className="relative w-full">
      ...
      >
        <span className={clsx(displayValue ? 'text-foreground' : 'text-muted-foreground')}>
      ...
      {isOpen && (
        <div className="absolute z-50 w-full mt-1 glass rounded-lg border border-border shadow-lg">
          <div className="py-1">
...
export function SelectContent({ children }: SelectContentProps) {
  return (
    <div className="glass rounded-lg border border-border shadow-lg py-1">
      {children}
    </div>
  );
}
...
export function SelectItem({ value, children, onSelect }: SelectItemProps & { onSelect?: (value: string, display: string) => void }) {
  ...
  return (
    <button
      type="button"
      className="w-full px-3 py-2 text-left text-muted-foreground hover:bg-primary-500/20 hover:text-foreground transition-colors focus:outline-none focus:bg-primary-500/20"
      onClick={handleClick}
    >
      {children}
    </button>
