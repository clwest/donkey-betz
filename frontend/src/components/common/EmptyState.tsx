...
const EmptyState: React.FC<EmptyStateProps> = ({ type, onAction }) => {
  const getEmptyStateContent = () => {
    switch (type) {
      case 'knowledge':
        return {
          icon: <Upload className="w-16 h-16 text-muted-foreground dark:text-gray-600" />,
      ...
      case 'gallery':
        return {
          icon: <Image className="w-16 h-16 text-muted-foreground dark:text-gray-600" />,
      ...
      case 'content':
        return {
          icon: <FileText className="w-16 h-16 text-muted-foreground dark:text-gray-600" />,
      ...
      case 'memories':
        return {
          icon: <Sparkles className="w-16 h-16 text-muted-foreground dark:text-gray-600" />,
      ...
      case 'conversations':
        return {
          icon: <Mic className="w-16 h-16 text-muted-foreground dark:text-gray-600" />,
      ...
      default:
        return {
          icon: <BookOpen className="w-16 h-16 text-muted-foreground dark:text-gray-600" />,
          title: "Get Started",
  ...
  return (
    <div className="flex flex-col items-center justify-center py-12 px-4">
      <div className="text-center max-w-md">
        {content.icon}
        <h3 className="mt-4 text-xl font-semibold text-gray-900 dark:text-foreground">
          {content.title}
        </h3>
        ...
        {onAction && (
          <button
            onClick={onAction}
            className="mt-6 px-6 py-3 bg-gradient-to-r from-blue-500 to-purple-500 text-foreground rounded-lg font-medium hover:from-blue-600 hover:to-purple-600 transition-colors"
          >
            {content.actionText}
          </button>
        )}
        ...
        {content.tips.length > 0 && (
          <div className="mt-8 text-left">
            <h4 className="text-sm font-medium text-gray-700 dark:text-muted-foreground mb-3">
              Quick Tips:
            </h4>
