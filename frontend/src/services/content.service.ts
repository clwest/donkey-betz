...
export const contentService = {
  ...
  async generateText(request: ContentGenerationRequest) {
    const { data } = await apiClient.post('/v1/content/create/', {
      content_type: 'text',
      ...request,
    });
    return data;
  ...
  async generateImageBatch(request: ImageGenerationRequest & { variations: number }) {
    const { data } = await apiClient.post('/v1/content/batch/', request);
    return data;
  },
  ...
  async imageToImage(baseImage: File, request: Omit<ImageGenerationRequest, 'prompt'> & { prompt?: string }) {
    ...
    const { data } = await apiClient.post('/v1/content/img2img/', formData, {
  ...
  async generateTextToVideo(request: VideoGenerationRequest): Promise<VideoResponse> {
    const { data } = await apiClient.post('/v1/video/text-to-video/', request);
    return data;
  },

  // Generate image-to-video
  async generateImageToVideo(request: VideoGenerationRequest): Promise<VideoResponse> {
    const { data } = await apiClient.post('/v1/video/image-to-video/', request);
    return data;
  },

  // Get video generation status
  async getVideoStatus(taskId: string): Promise<VideoStatusResponse> {
    const { data } = await apiClient.get(`/v1/video/status/${taskId}/`);
    return data;
  },
  ...
  async getVideoDetail(contentId: number): Promise<{ video: any }> {
    const { data } = await apiClient.get(`/v1/video/${contentId}/`);
    return data;
  },
  ...
  async upscaleImage(image: File, factor: 2 | 4 = 2, creativity: 'conservative' | 'creative' = 'conservative') {
    ...
    const { data } = await apiClient.post('/v1/stability/upscale/', formData, {
      headers: { 'Content-Type': 'multipart/form-data' },
    });
    return data;
  },
  ...
  async removeBackground(image: File) {
    ...
    const { data } = await apiClient.post('/v1/stability/remove-background/', formData, {
      headers: { 'Content-Type': 'multipart/form-data' },
    });
    return data;
  },
  ...
  async inpaintImage(image: File, mask: File, prompt: string) {
    ...
    const { data } = await apiClient.post('/v1/stability/inpaint/', formData, {
      headers: { 'Content-Type': 'multipart/form-data' },
    });
    return data;
  },
  ...
  async outpaintImage(image: File, direction: 'up' | 'down' | 'left' | 'right' | 'all', prompt?: string) {
    ...
    const { data } = await apiClient.post('/v1/stability/outpaint/', formData, {
      headers: { 'Content-Type': 'multipart/form-data' },
    });
    return data;
  },
  ...
  async searchAndReplace(image: File, searchPrompt: string, replacePrompt: string) {
    ...
    const { data } = await apiClient.post('/v1/stability/search-replace/', formData, {
      headers: { 'Content-Type': 'multipart/form-data' },
    });
    return data;
  },
  ...
  async eraseObject(image: File, mask: File) {
    ...
    const { data } = await apiClient.post('/v1/stability/erase/', formData, {
      headers: { 'Content-Type': 'multipart/form-data' },
    });
    return data;
  },
  ...
  async sketchToImage(image: File, prompt: string) {
    ...
    const { data } = await apiClient.post('/v1/stability/sketch/', formData, {
      headers: { 'Content-Type': 'multipart/form-data' },
    });
    return data;
  },
  ...
  async generate3D(image: File) {
    ...
    const { data } = await apiClient.post('/v1/stability/3d/', formData, {
      headers: { 'Content-Type': 'multipart/form-data' },
    });
    return data;
  },
  ...
  async transcribeAudio(audioFile: File, outputType = 'transcript') {
    ...
    const { data } = await apiClient.post('/v1/voice/transcribe/', formData, {
      headers: { 'Content-Type': 'multipart/form-data' },
    });
    return data;
  },
  ...
  async formatConversation(transcriptId: string, speakers?: string[]) {
    const { data } = await apiClient.post('/v1/voice/format-conversation/', {
  ...
  async generateBlog(request: {
    ...
    const { data } = await apiClient.post('/v1/content/blog/generate/', request, {
  ...
  async generateSocialPosts(request: {
    ...
    const { data } = await apiClient.post('/v1/content/social/generate/', request, {
  ...
  async getSocialPostsList() {
    const { data } = await apiClient.get('/v1/content/social/list/');
    return data;
  },

  // Get specific social post details
  async getSocialPost(id: number) {
    const { data } = await apiClient.get(`/v1/content/social/${id}/`);
    return data;
  },

  // Custom Styles
  async getStyles() {
    const { data } = await apiClient.get('/v1/styles/');
    return data;
  },

  async getCustomStyles() {
    const { data } = await apiClient.get('/v1/custom-styles/');
    return data;
  },
  ...
  async createCustomStyle(style: {
    ...
    const { data } = await apiClient.post('/v1/custom-styles/', style);
    return data;
  },
  ...
  async getGalleryItems(limit = 50, content_type?: string) {
    ...
    const { data } = await apiClient.get(`/v1/gallery/?${params}`);
    return data;
  },

  async saveToGallery(contentId: string, title?: string, tags?: string[]) {
    const { data } = await apiClient.post('/v1/gallery/save/', {
  ...
  async saveImageToGallery(params: {
    ...
    const { data } = await apiClient.post('/v1/gallery/save/', params);
    return data;
  },
  ...
  async saveVideoToGallery(params: {
    ...
    const { data } = await apiClient.post('/v1/gallery/save-video/', params);
    return data;
  },
  ...
  async getVideoGallery(params?: {
    ...
    const { data } = await apiClient.get(`/v1/gallery/videos/?${queryParams}`);
    return data;
  },
  ...
  async createPodcast(podcastData: {
    ...
    const { data } = await apiClient.post('/v1/podcasts/', podcastData);
    return data;
  },
  ...
  async generatePodcast(podcastData: {
    ...
    const { data } = await apiClient.post('/v1/podcasts/generate/', podcastData);
    return data;
  },

  async generatePodcastScript(episodeId: number) {
    const { data } = await apiClient.post(`/v1/podcasts/${episodeId}/generate-full/`);
    return data;
  },

  async getPodcastList() {
    const { data } = await apiClient.get('/v1/podcasts/');
    return data;
  },
  ...
  async saveBlogPost(blogData: {
    ...
    const { data } = await apiClient.post('/v1/content/blog/save/', dataToSave);
    return data;
  },

  // Get saved blog posts
  async getBlogList() {
    const { data } = await apiClient.get('/v1/content/blog/list/');
    return data;
  },

  // Get specific blog post
  async getBlogPost(id: number) {
    const { data } = await apiClient.get(`/v1/content/blog/${id}/`);
    return data;
  },
  ...
  async updateBlogPost(id: number, blogData: {
    ...
    const { data } = await apiClient.put(`/v1/content/blog/${id}/`, blogData);
    return data;
  },
  ...
  async submitBlogFeedback(id: number, feedbackData: {
    ...
    const { data } = await apiClient.post(`/v1/content/blog/${id}/feedback/`, feedbackData);
    return data;
  },
  ...
  async deleteBlogPost(id: number) {
    ...
    const { data } = await apiClient.put(`/v1/content/blog/${id}/`, {
  ...
  async exportContent(contentId: string, format: 'png' | 'jpg' | 'pdf' | 'docx') {
    const { data } = await apiClient.post(`/v1/export/${contentId}/`, {
      format,
    }, {
      responseType: 'blob',
    });
  ...
  async generateSocialFromBlog(request: {
    ...
    const { data } = await apiClient.post('/v1/content/social/generate/', {
      topic: request.topic,
  ...
  async generatePodcastFromBlog(request: {
    ...
    const { data } = await apiClient.post('/v1/podcasts/generate/', {
      topic: request.title,
  ...
  async generateEbookFromBlog(request: {
    ...
    const { data } = await apiClient.post('/v1/ebooks/', {
