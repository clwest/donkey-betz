...
export const styleMemoryService = {
  ...
  async respondToSuggestion(suggestionId: string, response: 'used' | 'dismissed') {
    const { data } = await apiClient.post(`/v1/style-memory/suggestions/${suggestionId}/respond/`, {
      response,
    });
    return data;
  },
  ...
  async extractRecipe(contentId: string) {
    const { data } = await apiClient.get(`/v1/style-memory/recipe/${contentId}/`);
    return data;
  },

  // Search similar styles using semantic search
  async searchSimilarStyles(query: string, limit = 10) {
    const { data } = await apiClient.post('/v1/style-memory/search/', {
      query,
      limit,
    });
    return data;
  ...
  async batchInteractions(interactions: Array<{
    ...
    const { data } = await apiClient.post('/v1/style-memory/batch/', {
      interactions,
    });
    return data;
  },
  ...
  async getEvolutionTimeline(timeRange = '30d') {
    const { data } = await apiClient.get(`/v1/style-memory/evolution/?range=${timeRange}`);
    return data;
  },
};