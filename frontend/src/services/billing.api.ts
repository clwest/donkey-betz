...
class BillingAPI {
  ...
  async getPricingTiers(): Promise<PricingTier[]> {
    const response = await apiClient.get('/v1/billing/pricing/');
  ...
  async createCheckoutSession(request: CheckoutSessionRequest): Promise<CheckoutSessionResponse> {
    const response = await apiClient.post('/v1/billing/checkout/', request);
    return response.data;
  }

  async cancelSubscription(cancelImmediately: boolean = false): Promise<{ message: string; subscription: Subscription }> {
    const response = await apiClient.post('/v1/billing/subscription/cancel/', {
  ...
  async reactivateSubscription(): Promise<{ message: string; subscription: Subscription }> {
    const response = await apiClient.post('/v1/billing/subscription/reactivate/');
    return response.data;
  }
  ...
  async getUsageHistory(params: { 
    ...
    const response = await apiClient.get('/v1/billing/usage/', { params });
    return response.data;
  }

  async getBillingSummary(): Promise<BillingSummary> {
    const response = await apiClient.get('/v1/billing/summary/');
    return response.data;
  }
  ...
  async addPaymentMethod(paymentMethodData: {
    ...
    const response = await apiClient.post('/v1/billing/payment-methods/', paymentMethodData);
    return response.data;
  }
  ...
  async setDefaultPaymentMethod(paymentMethodId: number): Promise<PaymentMethod> {
    const response = await apiClient.patch(`/v1/billing/payment-methods/${paymentMethodId}/`, {
      is_default: true
    });
    return response.data;
  }
  ...
  async purchaseCredits(data: {
    ...
    const response = await apiClient.post('/v1/billing/credits/purchase/', data);
    return response.data;
  }
  ...
  async getAdminDashboard(days: number = 30): Promise<{
    ...
    const response = await apiClient.get('/v1/billing/admin/dashboard/', {
      params: { days }
    });
    return response.data;
  }
  ...
  async getAdminUsers(): Promise<Array<{
    ...
    const response = await apiClient.get('/v1/billing/admin/users/');
    return response.data;
  }
}
