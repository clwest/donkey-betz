...
export const voiceService = {
  ...
  async generateVoice(request: VoiceGenerationRequest): Promise<VoiceResponse> {
    const { data } = await apiClient.post('/v1/voice/generate/', request);
    return data;
  },

  // Get available voices
  async getVoices(): Promise<Voice[]> {
    const { data } = await apiClient.get('/v1/voice/voices/');
    return data.voices || [];
  },
  ...
  async generateVideoNarration(params: {
    ...
    const { data } = await apiClient.post('/v1/voice/video-narration/', params);
    return data;
  },
