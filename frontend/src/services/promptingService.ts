...
class PromptingService {
  async getSettings(): Promise<PromptingSettings> {
    const response = await apiClient.get('/v1/prompting/settings/');
    return response.data;
  }

  async updateSettings(settings: Partial<PromptingSettings>): Promise<any> {
    const response = await apiClient.put('/v1/prompting/settings/', settings);
    return response.data;
  }

  async getStats(days: number = 30): Promise<PromptingStats> {
    const response = await apiClient.get(`/v1/prompting/stats/?days=${days}`);
    return response.data;
  }
  ...
  async testEnhancement(params: {
    ...
    const response = await apiClient.post('/v1/prompting/test/', params);
    return response.data;
  }

  async getSuggestions(contentType: string = 'default'): Promise<EnhancementSuggestions> {
    const response = await apiClient.get(`/v1/prompting/suggestions/?content_type=${contentType}`);
    return response.data;
  }
}
