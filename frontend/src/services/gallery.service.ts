...
export const galleryService = {
  ...
  async getItems(filters: GalleryFilters = {}, page = 1, limit = 50): Promise<{
    ...
    const { data } = await apiClient.get(`/gallery/list/?${params}`);
    return {
  ...
  async saveItem(item: {
    ...
    const { data } = await apiClient.post('/gallery/', item);
    return data;
  },
  ...
  async toggleFavorite(id: string): Promise<GalleryItem> {
    const { data } = await apiClient.post(`/gallery/${id}/toggle-favorite/`);
    return data;
  },

  // Add/remove tags
  async updateTags(id: string, tags: string[]): Promise<GalleryItem> {
    const { data } = await apiClient.post(`/gallery/${id}/tags/`, { tags });
    return data;
  },
  ...
  async getMetadata(): Promise<{
    ...
    const { data } = await apiClient.get('/gallery/metadata/');
    return data;
  },
  ...
  async searchItems(query: string, filters: GalleryFilters = {}): Promise<{
    ...
    const { data } = await apiClient.get('/gallery/search/', {
  ...
  async getCollections(): Promise<GalleryCollection[]> {
    const { data } = await apiClient.get('/gallery/collections/');
    return data;
  },
  ...
  async createCollection(collection: {
    ...
    const { data } = await apiClient.post('/gallery/collections/', collection);
    return data;
  },

  async updateCollection(id: string, updates: Partial<GalleryCollection>): Promise<GalleryCollection> {
    const { data } = await apiClient.put(`/gallery/collections/${id}/`, updates);
    return data;
  },

  async deleteCollection(id: string): Promise<void> {
    await apiClient.delete(`/gallery/collections/${id}/`);
  },

  async addToCollection(collectionId: string, itemIds: string[]): Promise<GalleryCollection> {
    const { data } = await apiClient.post(`/gallery/collections/${collectionId}/add/`, {
      item_ids: itemIds,
    });
    return data;
  },
  ...
  async removeFromCollection(collectionId: string, itemIds: string[]): Promise<GalleryCollection> {
    const { data } = await apiClient.post(`/gallery/collections/${collectionId}/remove/`, {
      item_ids: itemIds,
    });
    return data;
  },
  ...
  async exportItems(itemIds: string[], format: 'zip' | 'pdf'): Promise<Blob> {
    const { data } = await apiClient.post('/gallery/export/', {
      item_ids: itemIds,
      format,
    }, {
      responseType: 'blob',
  ...
  async shareItems(itemIds: string[], options: {
    ...
    const { data } = await apiClient.post('/gallery/share/', {
      item_ids: itemIds,
      ...options,
    });
    return data;
  ...
  async getSharedItems(shareCode: string, password?: string): Promise<{
    ...
    const { data } = await apiClient.get(`/gallery/shared/${shareCode}/`, {
      params: { password },
    });
    return data;
  },
  ...
  async duplicateItem(id: string, title?: string): Promise<GalleryItem> {
    const { data } = await apiClient.post(`/gallery/${id}/duplicate/`, { title });
    return data;
  },

  // Generate variations
  async generateVariations(id: string, count = 3): Promise<GalleryItem[]> {
    const { data } = await apiClient.post(`/gallery/${id}/variations/`, { count });
    return data;
  },
  ...
  async cropImage(id: string, cropData: {
    ...
    const { data } = await apiClient.post(`/gallery/${id}/crop/`, cropData);
    return data;
  },
  ...
  async resizeImage(id: string, dimensions: {
    ...
    const { data } = await apiClient.post(`/gallery/${id}/resize/`, dimensions);
    return data;
  },
  ...
  async autoTag(id: string): Promise<{
    ...
    const { data } = await apiClient.post(`/gallery/${id}/auto-tag/`);
    return data;
  },
};