...
export const workflowService = {
  ...
  async scheduleWorkflow(id: string, schedule: any) {
    const { data } = await apiClient.post(`/v1/workflows/${id}/schedule/`, schedule);
    return data;
  },

  async removeSchedule(id: string) {
    await apiClient.delete(`/v1/workflows/${id}/schedule/`);
  },
  ...
  async getAvailableNodes() {
    const { data } = await apiClient.get('/v1/workflow-nodes/');
    return data;
  },

  async getNodeConfig(nodeType: string) {
    const { data } = await apiClient.get(`/v1/workflow-nodes/${nodeType}/config/`);
    return data;
  },
};