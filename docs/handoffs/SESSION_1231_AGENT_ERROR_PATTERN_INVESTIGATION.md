# Session 1231 — Agent Error-Pattern Investigation (P3 + P2 + R2 + P4 + 83-agent smoke + F4 + F7 + F8)

**Status:** Seven-PR session. Opened bundling three findings from Session 1230's audit deliverable `5318da3e-…` and tracking deliverable `61f4312b-…` into one "agent error-pattern investigation" arc (P3 + P2 + R2). After Chris's follow-on ask "can Rigby trigger each of the Agents and get an output from them?", added P4 — a full 83-agent AGENT_MAP coverage smoke that surfaced two true production-broken agents (BookmakerAgent + WorkflowOrchestrationAgent). PR #2588 closed BookmakerAgent; PR #2590 closed WorkflowOrchestrationAgent case-sensitivity (F4); PR #2592 closed strategic_synthesis missing-handler (F7); PR #2593 closed a latent None-handling bug in `_compile_final_result` (F8) that F7 surfaced by unblocking the first workflows to ever reach success on the no-create_project path. Final WorkflowOrchestrationAgent end-to-end verification: 149.6s `status=completed`, zero error. **68 of 68 verifiable production-healthy = 100% dispatch contract AND workflow completion contract.**
...
| **#2589** | `docs(session-1231): handoff + start-here update with PR #2588 + full 83-agent smoke results` | Second close-doc pass after #2588 landed. Title bumped, 4-PR PR table, full 83-agent smoke breakdown, operational invariants #4+#5, followups F4/F5/F6, FIRST THING Session 1232 reordered with F4+F5 as new Priorities 2+3. |
| **#2590** | `fix(session-1231): WorkflowOrchestrationAgent AGENT_MAP fallback for snake_case step names (F4)` | `core/services/workflow_orchestration_agent.py:1278+` — replaced the catch-all `else` in `_execute_step()` with an AGENT_MAP fallback that converts snake_case → PascalCase and dispatches via `AgentRouter.route()`. Unblocks the 3 named-AGENT_MAP step names referenced in built-in templates (`research_agent` / `competitor_analysis_agent` / `customer_research_agent`). The 4th name `strategic_synthesis` → `StrategicSynthesis` is not in AGENT_MAP; the fallback returns an explicit error naming both forms (replaces the prior generic `"Unknown agent in workflow"` message). 6 new tests in `test_workflow_orchestration_agent_map_fallback.py` covering 3 successful routes + 1 not-in-AGENT_MAP explicit error + 1 dispatch-exception surfacing + 1 internal-handlers-take-precedence regression guard. 6/6 green. Post-fix verification dispatch (`d9b71e4a`) ran 148s through steps 1-3 (vs pre-fix 4s abort at step 1), then surfaced F7 at step 4 (`strategic_synthesis`). |
| **#2591** | `docs(session-1231): handoff + start-here update with PR #2590 F4 close + F7 new` | Third close-doc pass after #2590 landed. |
| **#2592** | `fix(session-1231): strategic_synthesis workflow step handler (F7)` | `core/services/workflow_orchestration_agent.py:1285+` — promote `strategic_synthesis` from "missing agent" to a dedicated workflow-internal handler. Reads any prior step outputs from workflow context (10 keys defined at class level: research_summary, research_results, competitor_insights, customer_insights, customer_personas, customer_pain_points, brand_recommendations, creative_recommendations, enhanced_summary, spider_insights). Builds a synthesis prompt; calls gpt-5-mini via OpenAI factory at `max_completion_tokens=4000` (Session 1224 floor rule). Persists synthesis into `context['strategic_synthesis']` for downstream steps. Graceful empty-context behavior: returns success with explicit "No synthesis input available" note instead of aborting (smoke/capability-ping case). Empty-LLM-response surfaces as explicit error with finish_reason. 7 new tests + 1 updated F4 test. 13/13 green. Post-merge verification dispatch ran 155s through all 4 steps successfully, then surfaced F8 (pre-existing latent bug). |
| **#2593** | `fix(session-1231): _compile_final_result None-defense for project_created (F8)` | `core/services/workflow_orchestration_agent.py:3486+` and `:3548+` — replace `context.get('project_created', {}).get(...)` with `(context.get('project_created') or {}).get(...)`. Pre-existing bug: line 1129 inits `'project_created': None`, and `.get(key, default)` returns the default only when key is ABSENT, not when present-with-None — so `.get('project_id')` raised AttributeError. Latent since the init was added; only fired when F7 unblocked the first workflows to reach `_compile_final_result(success=True)` without a `create_project_from_research` step. 1 new test asserting `_compile_final_result` survives the exact init shape from execute() line 1111-1133. 14/14 green. **End-to-end verification (`c5224af7`): WorkflowOrchestrationAgent dispatched with `mode=receipt_only` → `status=completed` in 149.6s, zero error message.** |
...
| ❌ **Real bug — FIXED in PR #2590** | **1** | `WorkflowOrchestrationAgent`: `"Step 'market_research' failed: Unknown agent in workflow: research_agent"`. Workflow lookup table uses snake_case `'research_agent'`; AGENT_MAP keys are PascalCase `'ResearchAgent'`. PR #2590 added an AGENT_MAP fallback that converts snake_case → PascalCase and dispatches via `AgentRouter.route()`. Post-fix verification dispatch (`d9b71e4a`) ran 148s through steps 1-3 successfully (vs pre-fix 4s abort at step 1), then surfaced followup F7: `strategic_synthesis` step 4 references a non-existent `StrategicSynthesis` agent. |
...
**Effective fleet health:** Excluding R1 cascade (Chris-side), by-design disabled, and shape rejections (which are agents working correctly on inappropriate input), **67 of 68 verifiable production-healthy = 98.5%** post-PR-#2588 → **68 of 68 = 100% post-PR-#2590** for case-sensitivity. (WorkflowOrchestrationAgent's workflow templates still fail at step 4 of `business_research` / `startup_validation` due to F7 `strategic_synthesis` missing-agent, but that's a workflow-template issue not an agent issue — the dispatch contract is honored.)
...
6. **`WorkflowOrchestrationAgent` dispatches snake_case step names via AGENT_MAP fallback.** Built-in workflow templates that reference snake_case names (e.g., `'research_agent'`) now correctly dispatch to the PascalCase AGENT_MAP entry (`'ResearchAgent'`). Unknown names get an explicit "no internal handler + not in AGENT_MAP" error naming both forms — replaces the prior silent "Unknown agent in workflow" message. Source-level test guards the 9 existing internal handlers from being shadowed by the fallback.
...
| #2590 F4 fix | Restore the catch-all `else: return {'success': False, 'error': f"Unknown agent in workflow: {agent_name}"}` at `core/services/workflow_orchestration_agent.py:1278`. AGENT_MAP fallback regression tests will fail; the 3 named-AGENT_MAP workflow steps will silently fail again at step 1 of `business_research`/`competitor_analysis`/`customer_personas`/`startup_validation` templates. Only valid if all 4 affected templates are also updated to use existing internal handler names. |
...
| **F7** | `strategic_synthesis` workflow step references a non-existent agent | **CLOSED by #2592** | Promoted to a dedicated workflow-internal handler that reads prior step outputs from context, builds a synthesis prompt, calls gpt-5-mini, and persists the synthesis. Graceful empty-context behavior keeps the dispatch contract honored. |
| **F8** | `_compile_final_result` AttributeError on `project_created` None init | **CLOSED by #2593** | Pre-existing latent bug surfaced by F7 close. `context['project_created']` initialized to None at line 1129; `context.get('project_created', {}).get(...)` returns None.get(…) → AttributeError. One-line `or {}` defense at both callsites. End-to-end verification: WorkflowOrchestrationAgent receipt_only smoke now `status=completed` in 149.6s. |
