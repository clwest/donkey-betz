...
> **Companion (original proposal):** [`docs/MERGE_PROPOSAL_CHARACTER_OS.md`](MERGE_PROPOSAL_CHARACTER_OS.md) — read both before deciding.
> **Preserved because:** documents the native-integration counter-proposal for the Character OS merge (more invasive but architecturally cleaner than the sidecar approach); useful as design history when (and if) Chris reopens the merge in response to customer demand. Do NOT execute the proposal as-written without explicit unpark signal.
