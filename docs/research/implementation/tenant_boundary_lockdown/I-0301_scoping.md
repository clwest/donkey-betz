...
| **Phase 3 — HTTP Remediation** | **OPEN 2026-07-10** — Stage 1 (substrate) in progress. Chris authorized open per Stage 1 SIGN. Stage 1: `core/security/` package + DRF handler + Django middleware + tests + CI workflow; NO endpoint changes. Stage 2: Bucket B remediation. Stage 3: Bucket C caller-graph + Bucket D evidence. | Phase 2 close ✅ | All 27+ endpoints remediated per bucket; base DRF exception handler + Django middleware + template overrides + regression suite scaffolded conforming to safety contract §8; first tests pass; CI-blocking |
