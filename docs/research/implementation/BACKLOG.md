...
| `IB-1799-T1-02` | 1799 §1 axis C + S1703 F1 | xx99_followon_T1 | Wire PA-invoked agents → `AgentExecution` writes (PA tool dispatch currently bypasses `dispatch_agent` code path); ADR-B specifies PA↔LLMCallEvent correlation contract per F5 fold | observability,pa,agents | NEEDS_ADR | RATIFIED_ADR | IN_ARC (I-0100) — code-merged, flag-flip-blocked | RATIFIED (band) | `core/services/unified_pa_entrypoint.py` (write helpers + create/finalize insertions), `core/migrations/0377_arc_i0100_p4_canonical_pa_agent.py` (canonical PA Agent row per Sub-option 1(i)), `core/settings.py` (flag definition), `core/tests/test_pa_agent_execution_write.py` (10 tests). `pr_refs: #2955` merged 2026-07-06. `adr_ref: ADR-0002` (accepted 2026-07-06). Feature flag `PA_AGENT_EXECUTION_WRITE_ENABLED` default OFF. **Distinct from IB-1799-T1-01 SHIPPED classification:** P4 code-merged BUT NOT yet SHIPPED per IOS §2.2 status semantics — three STOP CONDITIONS block flag flip per #2955 PR body: (1) ~150-site `AgentExecution` consumer sweep (larger than ADR-0002 F1 estimate of ~25); (2) early-return path finalization decision (injection blocked, triage mode); (3) `LLMCallEvent.execution_id` end-to-end join verification post-flag-flip. Runtime fix DORMANT until all three stop conditions discharge. Row flips `IN_ARC → SHIPPED` only when Chris ratifies the stop-condition discharge + directs the flag flip. Scoping doc: §3.1 + §5 P4. |
| `IB-1799-T1-03` | 1799 §1 axis E + S1705 F1 | xx99_followon_T1 | Discharge R3 delegation-handler dormancy via staged unlock of `RIGBY_DELEGATION_ENABLED` (sole runtime gate per ADR-0003 §2.1 terminology binding; `MISSION_RUNNER_ENABLED` is docs-only legacy slug — NO CODE reference) — Phase 1 shadow-audit-synthetic (`manage.py delegation_lifecycle_smoke_test`) + Phase 2 direct full-enable per ADR-0003 §3.1 Option 5 hybrid | observability,employee_os | NEEDS_ADR | RATIFIED_ADR | IN_ARC (I-0100) — code-merged, flag-flip-blocked | RATIFIED (band) | `core/signals/rigby_delegation_signals.py` (handler; R3 dormancy target), `core/services/rigby_mission_delegation.py` (`delegate_work_item`), `core/settings.py:138-140` (RIGBY_DELEGATION_ENABLED flag), `core/management/commands/delegation_lifecycle_smoke_test.py`, `core/services/delegation_auto_disable.py`, `core/tests/test_delegation_lifecycle_smoke_test.py`. `pr_refs: #2957` merged 2026-07-06. `adr_ref: ADR-0003`. Feature flag `RIGBY_DELEGATION_ENABLED` default OFF. Stage 3 pre-flight all 6 §3.3.a assumption-lock gates PASS at 2026-07-06 (A1 cardinality=1 + A2 cadence 0/day + A3 handler unchanged + A4 sole gate + A5 MissionRunner contract + A6 baseline captured). Phase 1 smoke command verified end-to-end (5 lifecycle labels + idempotency + savepoint rollback). **Matches P4 pattern: code-merged, runtime intentionally blocked by deployment gates.** Row flips `IN_ARC → SHIPPED` only when Chris ratifies 4 STOP CONDITIONS discharge + directs flag flip: (1) monitoring surface integration (`MONITORING_SURFACE_INTEGRATED=True`); (2) Phase 2 evidence bundle review per §3.1 F1; (3) re-verification of A1-A6 at flag-flip time; (4) ratified §3.1 F1 entry gate. Naming correction per ADR-0003 §3.6 applied — single-flag scope. |
