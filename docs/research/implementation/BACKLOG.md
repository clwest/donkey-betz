...
| 2026-07-07 | **I-0100** — Stage 6 close (LOCAL) | Arc-terminal; FULLY CLOSED under LOCAL operating model | All 3 T1 rows in terminal disposition: `IB-1799-T1-01` SHIPPED (#2954 flag OFF); `IB-1799-T1-02` LOCAL_ACCEPTED / PRODUCTION_DEFERRED (#2955 + #2970 acceptance #2973); `IB-1799-T1-03` LOCAL_ACCEPTED / PRODUCTION_DEFERRED (#2957 + #2974 acceptance #2975). Both runtime flags env-driven `false`. Operating model LOCAL-only per PR #2972. Close doc: `I-010099_observability_spine_implementation_close.md` (PR #2976). | (Arc I-0100 CLOSED; scoping doc `status: closed-local`) |
| 2026-07-07 | **I-0200** — Second-arc selection SIGN routed via Rigby | Selection ratification pin lifecycle | Rigby SIGN-with-edits MED confidence on second-arc pick; 8 folds F1-F8. Selection SIGN pin `pa-6a4e2eff5594486b` (label `ios-arc-open-I-0200-sign-review`) minted + retired 2026-07-07 (updated_count=4 at retire). Ratification record: `RATIFICATION_2026-07-07_second_arc_I-0200.md`. | (RATIFICATION_2026-07-07 committed alongside scoping doc) |
| 2026-07-07 | **I-0200** — Stage 1 arc-open bundle (contingency-gated seed) | Stage 1 opened; Rigby SIGN Cycle 1 pending on scoping doc | **Contingency-gated seed per Chris "Agree All" 2026-07-07:** default seed `IB-2199-T0-01` (RAG corpus substrate maturity gradient) IF Stage 1 confirms severability from `IB-2199-BOR-01`; ELSE auto-switch to `IB-1999-T0-01` (Authority per-plane posture). Neither row flipped to `IN_ARC` at this arc-open — flip pending Stage 1 severability determination. Companion evaluation candidates recorded (default: `IB-2199-T1-01` + `IB-CXP10-T1-03`; fallback: `IB-1999-T1-01` + `IB-1999-T1-02` + `IB-1999-T1-03` + `IB-CXP10-T1-01`). No auto-admission per Axis 4. Arc-scoped pin `pa-1b76ee75adbf4031` (label `ios-arc-open-I-0200`) minted 2026-07-07; `tools/pa_local.sh --conversation` rotated. LOCAL-only guardrail carried forward per PR #2972. | [rag_corpus_substrate_maturity/I-0200_scoping.md](rag_corpus_substrate_maturity/I-0200_scoping.md) (default-path folder; renames to `authority_per_plane_posture/` if severability determination flips to fallback) |
| 2026-07-07 | **I-0200** — P1 severability determination CLOSED SEVERABLE | Stage 1 exit-gate cleared → P1 close; Stage 2 opens on separate Chris directive (not on P1 PR merge per Chris ratification directive) | **P1 SEVERABILITY DETERMINATION SEVERABLE per Chris "Agree All" 2026-07-07.** All three Q-Sev answered YES with F12 verbatim evidence citations (Q-Sev-1 cites 2199 xx99 §1 line 74; Q-Sev-2 cites §1 line 84; Q-Sev-3 cites §1 line 76). F11 UNKNOWN operational check clean under PROVISIONAL ADR shape. Rigby SIGN Cycle 1 SIGN-with-edits MED-HIGH; F15 (Stage 5 verification scope documentation-cross-check-only lock; runtime probes = BOR-01 out-of-scope) + F16 (companion outcomes NOT re-ratified by P1) applied pre-Chris-ratification. **`IB-2199-T0-01` flips `TRIAGED → IN_ARC (I-0200)`.** `IB-1999-T0-01` remains `TRIAGED` (fallback path not triggered). No folder rename. Stage 2 authoring constraints ratified: PROVISIONAL ADR marker (Option a or b); explicit "BOR-01 discharge is Stage-2-post-ratification requirement" clause; T-slot execution PRs (T18/T19/T21/T22/T13/T26a/T27/T29) named as post-arc requirements; no runtime enforcement claims; Stage 5 verification scope = documentation cross-check only. Companion-row admission still deferred per Chris directive "Do not admit companion rows." Determination doc: [rag_corpus_substrate_maturity/I-0200_severability_determination.md](rag_corpus_substrate_maturity/I-0200_severability_determination.md). Merged 2026-07-07 as `c39db4394` via PR #2982 (superseded auto-closed #2981). | (P1 close PR carries determination doc + BACKLOG flip + scoping doc §10 P1-close status updates) |
| 2026-07-07 | **I-0200** — Stage 2 opened + Design-Prep authored + Rigby SIGN-with-edits MED-HIGH + Chris "Agree All" | Stage 2 `stage_state: exit-gate-cleared → design-prep-in-flight` per IOS §4.3.0 v1.5; Stage 2 Entry gate satisfied per §4.3 clause 2(a) Chris explicit directive | **Design-prep first-class artifact `I-0200_design_prep_rag_corpus_substrate_maturity.md` authored per IOS v1.5 §4.3.a mandatory rule + Chris-directed arc-scoped IOS §14.2 workflow refinement (SIGN design-prep BEFORE ADR body drafting — deviates from §4.3.a default; recorded in design-prep §9.2 Provenance).** Rigby SIGN Cycle 1 SIGN-with-edits MED-HIGH overall (Q1 SIGN-clean 0.86 + Q2 SIGN-with-edits 0.78 + Q3 SIGN-clean 0.88 + Q4 SIGN-with-edits 0.74); folds F17 (C9 grep-discoverability downgraded to recommended-ops-constraint) + F18 (Stage 5 verification interface replaced deliverable_tool.list with tool-realistic repo_tool.search + search_docs + kb_tool + repo_tool.read_file) applied pre-Chris-ratification. Chris "Agree All" 2026-07-07 ratified: Option 1 (verbatim 2199 §5.1 seam statement + two-field PROVISIONAL frontmatter `status: accepted` + `provisional: true` + `provisional_reason` + per-slot enumerated T-slots T22/T13/T18/T19/T21/T26a/T27/T29); C1–C12 constraints with C9 as recommended ops constraint; §9.2 workflow-deviation recording accepted; ADR-0004 body drafting authorized post-PR-merge. **`IB-2199-T0-01` remains `IN_ARC (I-0200)`** (no status change at design-prep close). Companion rows NOT admitted per Chris directive carry-forward. No runtime code; no flag changes; no ADR body authored. Codification candidates surfaced (single-trigger): CC-1 arc-scoped workflow overrides + CC-2 PROVISIONAL ADR pattern selection + CC-3 docs-only ADR "code state" definition. Design-prep doc: [rag_corpus_substrate_maturity/I-0200_design_prep_rag_corpus_substrate_maturity.md](rag_corpus_substrate_maturity/I-0200_design_prep_rag_corpus_substrate_maturity.md). Merged 2026-07-07 as `c87878dd` via PR #2983. | (design-prep PR carries new design-prep doc + scoping doc frontmatter Stage 2 flip + §10 checklist updates + cascade artifacts) |
| 2026-07-07 | **I-0200** — ADR-0004 body ratified + `IB-2199-T0-01` `IN_ARC → SHIPPED` | Stage 2 ADR authoring closed per IOS §4.3 Stage 2 exit gate; Stage 3 SKIPPED per docs-only scope + F15 lock; Stage 5 verification bounded to documentation cross-check per F15 + F18; Stage 6 close doc next | **ADR-0004 `rag-corpus-substrate-maturity-gradient` (PROVISIONAL) authored + ratified 2026-07-07 per ratified Option 1 (verbatim 2199 §1 line 74 seam statement + two-field PROVISIONAL frontmatter `status: accepted` + `provisional: true` + `provisional_reason` + per-slot enumerated T-slots T22/T13/T18/T19/T21/T26a/T27/T29 in §4 Consequences + BOR-01-post-ratification clause + no-runtime-enforcement claims + Stage 5 documentation cross-check only).** Rigby SIGN Cycle 1 SIGN-with-edits MED-HIGH overall (Q1 SIGN-with-edits 0.84 + Q2 SIGN-clean 0.86 + Q3 SIGN-with-edits 0.77 + Q4 SIGN-with-edits 0.74); folds F19 (§3.1 strict-verbatim seam — remove outer double-quotes) + F20 (§4.4 forward-compat rule — consumers MUST ignore unknown frontmatter keys) + F21 (§6 rollback step 4 de-speculate cleanup command + soften retrieval-impact claim) applied pre-Chris-ratification. Chris "Agree All" 2026-07-07 ratified all three folds + Option 1 body + successor-trigger policy (BOR-01 discharge OR any listed T-slot ship flips PROVISIONAL). **`IB-2199-T0-01` flips `IN_ARC (I-0200) → SHIPPED` with `adr_ref: ADR-0004`; `pr_refs: #2984`.** Companion rows NOT admitted per Chris directive carry-forward. No runtime code; no flag changes; no production access. Codification candidates surfaced (single-trigger): CC-1 arc-scoped workflow overrides (SECOND application via ADR body SIGN following design-prep SIGN); CC-2 PROVISIONAL ADR pattern (FIRST live ratification of two-field pattern in ADR corpus); CC-4 additive-frontmatter-forward-compat rule (F20; consumers MUST ignore unknown keys — could codify as ADR-0001 amendment if Chris directs); CC-5 §3.1 strict-verbatim discipline for research→ADR content codification (F19). ADR-0004 doc: [../../adr/ADR-0004-rag-corpus-substrate-maturity-gradient.md](../../adr/ADR-0004-rag-corpus-substrate-maturity-gradient.md). Merged 2026-07-07 as `84b45a11` via PR #2984. | (ADR-0004 PR carries new ADR file + scoping doc §10 update + BACKLOG flip + OPEN_ARCS + 00-START refresh + cascade artifacts) |
| 2026-07-07 | **I-0200** — Stage 6 close (LOCAL) | Arc-terminal; FULLY CLOSED under LOCAL operating model (mirrors Arc I-0100 close pattern PR #2976); Stage 3 SKIPPED per docs-only scope; Stage 4 merged into Stage 2 (ADR PR was shipping vehicle); Stage 5 light-verify at close time bounded to documentation cross-check per F15+F18 | **Arc I-0200 CLOSED per IOS §4.3 Stage 6.** Canonical close doc `I-020099_rag_corpus_substrate_maturity_implementation_close.md` drafted (~290 lines; §1-§10 mirrors Playbook §11.3 xx99 with Executive Summary + What This Arc Shipped + Anchor-Updates + Cross-Cutting Patterns + Deferred/Blocked findings + Implementation Debt/Drift + Cross-Domain Audit §14 Delta + What This Arc Taught Us + Provenance + Stage-6 housekeeping). `cross_domain_integration_audit.md §14.18` appended (FIRST implementation-arc §14 entry in the audit). Scoping doc frontmatter `stage: 2 → 6`, `stage_state: exit-gate-cleared → awaiting-close` per IOS §4.3.0 v1.5. `IB-2199-T0-01` unchanged (`SHIPPED (I-0200)` from PR #2984). Companion rows NOT admitted (Chris directive carry-forward). No runtime code; no flag changes; no production access. Drift items recorded in close doc §6: (a) scoping stage_state drift fixed in this bundle; (b) design-prep §7.1(iv) mis-citation recorded not amended; (c) CC-1 two-application-within-arc ambiguity recorded; (d) CC-4 ADR-0001 forward-compat rule candidate recorded; (e) Arc I-0100 §14 audit-refresh gap noted for record (backfill deferred). Ratified fold count arc-wide: **21 folds F1-F21 across 4 Chris "Agree All" events on 2026-07-07** (selection + Stage 1 exit + P1 SEVERABLE + Stage 2 design-prep + ADR body). Arc-scoped SIGN pin `pa-1b76ee75adbf4031` retired via `session_tool.retire force=true` per §15.14 (mirrors Arc I-0100 PR #2977 pattern); `tools/pa_local.sh --conversation` rotated back to paused-research T4 pin `pa-44a6eb70d8814e34` per §15.14 restoration rule (mirrors Arc I-0100 PR #2978 pattern). Rigby SIGN Cycle 1 on close doc mandatory per IOS §4.3 Stage 6 item 7 + §7.2 Table. Close doc: [rag_corpus_substrate_maturity/I-020099_rag_corpus_substrate_maturity_implementation_close.md](rag_corpus_substrate_maturity/I-020099_rag_corpus_substrate_maturity_implementation_close.md). | (Stage 6 close PR carries close doc + audit §14.18 refresh + scoping doc frontmatter flip + BACKLOG arc-history row + OPEN_ARCS In-progress→Closed + 00-START refresh + wrapper rotation + cascade artifacts) |
...
| intake_id | source_ref | source_type | title | affected_domains | risk_class | design_state | status | chris_gate | affected_surfaces |
|---|---|---|---|---|---|---|---|---|---|
| `IB-1999-T0-01` | 1999 §17.1 | xx99_followon_T0 | Ratify Authority Enforcement per-plane posture (5 PERMEABLE-BROKEN + 2 STRUCTURAL-DROP + 1 CLEAN) | authority_enforcement | NEEDS_ADR | SPEC_COMPLETE | TRIAGED (Arc I-0200 fallback path NOT TRIGGERED at P1 close 2026-07-07 — SEVERABLE determination held per Chris "Agree All"; row remains available for a future arc as a standalone T0 posture ADR) | RATIFIED (band) | `docs/adr/`, per-plane register |
...
| intake_id | source_ref | source_type | title | affected_domains | risk_class | design_state | status | chris_gate | affected_surfaces |
|---|---|---|---|---|---|---|---|---|---|
| `IB-2199-T0-01` | 2199 §1 | xx99_followon_T0 | Ratify RAG corpus substrate maturity gradient (spec-complete/execution-pending) | rag,memory | NEEDS_ADR | RATIFIED_ADR | SHIPPED (I-0200) — flipped `IN_ARC (I-0200) → SHIPPED` 2026-07-07 at ADR-0004 PR merge per IOS §2.2 v1.4 Discipline B inline syntax. `adr_ref: ADR-0004` (`docs/adr/ADR-0004-rag-corpus-substrate-maturity-gradient.md`); `pr_refs: #2984` (amended pre-merge with actual PR number). Chris "Agree All" 2026-07-07 ratified Option 1 (verbatim seam + two-field PROVISIONAL + per-slot T-slots). Post-arc successor triggers per ADR-0004 §4.2: BOR-01 discharge OR any T-slot ship (T22/T13/T18/T19/T21/T26a/T27/T29). | RATIFIED (band + P1 SEVERABLE 2026-07-07 + ADR-0004 accepted 2026-07-07) | `docs/adr/ADR-0004-rag-corpus-substrate-maturity-gradient.md` (PROVISIONAL — successor triggered by BOR-01 discharge OR any listed T-slot ship) |
