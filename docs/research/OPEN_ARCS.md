...
authority: state
session_added: 1279
last_updated: 2026-07-01 (S1303 close — Group 1300 current-child advanced to S1303 SIGN-clean commit-gated + S1304 queued)
...
|-------|--------|-------|-----------|----------------|---------------|---------------|--------------|---------------|
| **1300** | Memory / Knowledge / Embeddings | in-progress | `pa-aa54193f240f4846` | `1300_memory_domain_scoping.md` (S1300) | `1303_memory_conversational_thread_memory_audit.md` (S1303, SIGN-clean 2026-07-01 via fresh isolation pin `pa-23a38300dd84bae2` after 12-edit fold + 1-cycle verification pass; commit-gated on Chris per playbook §16). Prior children: `1301_memory_rag_retrieval_lanes_audit.md` (S1301, closed on `main` via PRs #2775 + #2776); `1302_memory_persistence_architecture_audit.md` (S1302, closed on `main` via PR #2777 = `c053272a`). Next: S1304 queued. | 2026-07-01 | none | **S1304 Documentation Corpus ↔ RAG Boundary (Categories E ↔ D).** Inherits provenance-boundary questions from S1301 (§14.2 silent-failure surface + §19 downstream routing) + S1302 (§17.3 name-collision resolution as boundary pattern) + S1303 (§9 Cat F ↔ Cat D OBSERVED GAP + §19 R3 turn-context → RAG enrichment design). Scope per parent §5 P4 slot: integration lens between §3.15 (Docs Corpus) and §3.14 (RAG); smaller scope than P1-P3; benefits from §3.14 audit landing first (S1301 shipped). Known drift entering: 464 UNKNOWN / 2156 docs = 21.5% coverage gap (S1301 §14.2); provenance filter treats "path not in index" as `excluded_missing_provenance` by design; two provenance systems coexist without integration (row-level `DocumentEmbedding.source_type` + `ingested_via` never read by any retrieval path). |
