...
- **Containment mechanism:** batch 4 in-scope actions are all pure-ORM reads. `agent_conversations` + `workflows` + `stats` are direct-in-handler; the 4 delegated READ actions bounce through `_handle_initiative` which is itself pure-ORM for READ paths (verified indirectly via harness dispatch success + 0 external I/O signals in logs).
- **Dual-gate protection on mutations:** `action_item_cleanup` + `bulk_cleanup` use Session 1228 PR-A dual-gate (dry_run + confirm both default False; both must flip). Defense-in-depth against GPT-5.2 autofill-false optional booleans.
