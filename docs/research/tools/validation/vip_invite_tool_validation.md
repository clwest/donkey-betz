...
- **Handler exception:** caught at line 1016, logged via `logger.error("[VIP_INVITE] Error: {e}", exc_info=True)`, returns `{"error": <str>}`. No `error_code` field — **legacy-error envelope** (18th corroborating instance post-S2921's 16-instance count; podcast_tool in this same batch is the 17th). Substrate arc still gated on explicit Chris directive per 00-START forbidden-list.
