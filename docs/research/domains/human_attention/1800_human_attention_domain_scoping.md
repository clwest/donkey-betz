...
|-----------|--------------------------------|-----------|
| **`HAI_item_id`** | `HumanAttentionItem.id` UUID PK — per-attention-item identifier. Written by HAI producer agents (10+ producer sites across signal / revenue / verification paths). **VERIFIED-AT-CHILD S1801:** HAI_item_id CONFIRMED as bona fide cross-system correlation primitive at HEAD `eef2280f` — used across **8-10 distinct domains** (mission_control_executor + orchestration_approval + ops_autopilot verification+core + gate_progression_pipeline + opportunity_execution_pipeline + spider_action_pipeline + implementation_executor + feedback_processing + orchestration model + td_handlers_ops). **Producer surface:** 43 distinct code paths (25 direct + 8 bridge helpers + 10 bridge-using callers) — S1273 §3.16 "20+" CONFIRMED-AND-EXCEEDED. **Retention posture VERIFIED:** SAVED-FOREVER + BOUNDED-HARD-DELETE only for spider_action 6h + arbitrage 12h + [Learned] junk via `_impl_cleanup_boardroom_junk` @ `core/tasks_ops.py:39-97`; DIVERGENT from LLMCallEvent 30-day CELERY_TASK_EVENT_RETENTION_DAYS baseline. **MC-3 F5 correlation-primitive HYPOTHESIS box discipline SECOND application completed at S1801 — two-triggers threshold MET; CODIFICATION-READY promotion path pending one more durability check at S1802 P2 Cat B.** Full evidence: S1801 §9 + §10 + §15.7 F6/D7 + §20.8 appendix. | ~~P1 (Cat A) verifies coverage + producer inventory + retention.~~ **VERIFIED-AT-CHILD S1801 P1 Cat A.** |
| **`feedback_record_id`** | `HumanFeedbackRecord.id` — per-feedback-record UUID. Written by `HumanInterfaceService.record_decision()`. Points at HAI via FK. **HYPOTHESIS:** feedback_record_id is the trigger event for FeedbackProcessor post_save signal; classified positive/negative; consumed once then `fed_to_ml=True`. **HYPOTHESIS REMAINS at S1802 close (partial verification):** S1802 P2 Cat B audit @ HEAD `9885ab01` confirmed (a) TWO writers not one (`record_decision` @ `human_interface_service.py:325` AND `auto_approve_item` @ `human_attention_lifecycle.py:325`); (b) trigger event VERIFIED (signal @ `models_feedback_processing.py:372-392` fires on every HFR create); (c) classification VERIFIED (inline @ `:156-157` — not a standalone `classify_positive_negative` method); (d) `fed_to_ml=True` progression VERIFIED-PARTIAL (`_feed_to_ml` @ `:756-759` flips True from `record_decision` conditional path ONLY — auto-approve path never flips it, Q2 orphan case); (e) **CROSS-SYSTEM PRIMITIVE STATUS: DISPROVEN** — grep of `feedback_record_id|HumanFeedbackRecord\.id|feedback_record\b` across all `.py` files at HEAD returns ZERO production hits outside Cat B (contrast S1801 HAI_item_id 8-10 domains). `feedback_record_id` is DOMAIN-INTERNAL only. **MC-3 CODIFICATION-READY promotion path DOES NOT advance at S1802 close.** See S1802 §14 F5 + §20.7. | ~~P2 (Cat B) verifies classifier + fed_to_ml lifecycle + missed-consumption cases.~~ **VERIFIED-PARTIAL-AT-CHILD S1802 P2 Cat B** — trigger + classifier + fed_to_ml VERIFIED (with Q2 orphan case for auto-approve); cross-system-primitive DISPROVEN. |
| **`learning_event_id` / `agent_learning_id`** | `AgentLearning.id` UUID (Group 1300 territory) — per-learning-event identifier. Written by FeedbackProcessor for HAI round-trip + written by 10+ LearningBridges for autonomous signals. **HYPOTHESIS:** learning_event_id is a cross-source primitive whose provenance (HAI-mediated vs bridge-autonomous) is NOT tagged at schema level. Would need `source_kind` enum to distinguish. **HYPOTHESIS REMAINS at S1803 close (SECOND CONSECUTIVE DISPROVED-CROSS-SYSTEM outcome after S1802):** S1803 P3 Cat C audit @ HEAD `69cf2dd1` verified `learning_event_id` as ID-primitive: 4 hits across 2 files, both in `ai_core/intelligence/` (`persistent_learning_engine.py:88, :101, :264` + `embedding_generator.py:382`); zero cross-domain reads. Serialization-boundary grep for `learning_event` as string-tag (per Rigby SIGN cycle 1 fold #4) surfaced 6 additional hits across 4 files but all TYPE-STRING TAGS, not ID-primitives — verdict on primitive holds DOMAIN-INTERNAL. **CROSS-SYSTEM PRIMITIVE STATUS: DISPROVEN.** **S1803 additionally confirmed 100% AUTONOMOUS bridge writer plane** — zero of 9 core `LearningBridge` subclasses touch HAI plane; all write `UserAgentLearning` @ 22 sites + 4 non-bridge writers. **`source_kind` absence VERIFIED** at HEAD (no provenance tagging on AgentLearning/UserAgentLearning) — R8 post-arc joint Group 1300 + Group 1800 schema-change ADR. **MC-3 CODIFICATION-READY promotion path DOES NOT advance at S1803 close** — SECOND consecutive negative outcome (matching S1802 feedback_record_id pattern). See S1803 §14 F5 + §20.7 + §20.6 Rigby SIGN fold record. | ~~P3 (Cat C) verifies bridge writer inventory + source_kind absence + cross-bridge dedup.~~ **VERIFIED-PARTIAL-AT-CHILD S1803 P3 Cat C** — bridge writer inventory VERIFIED (9 subclasses + 4 non-bridge writers); source_kind absence VERIFIED; cross-bridge dedup catalogued; cross-system-primitive DISPROVEN. |
| **`user_pref_id`** | `HumanPreference.id` — per-user preference row. Written by `update_learned_stats()`. **HYPOTHESIS:** F5 bug — `topic_weights` / `source_weights` set locally but never `.save()`d; verify F5 root cause + downstream consumers (are there ANY readers of topic_weights at HEAD?). **HYPOTHESIS REMAINS at S1804 close (THIRD CONSECUTIVE DISPROVED-CROSS-SYSTEM outcome after S1802 + S1803):** S1804 P4 Cat D audit @ HEAD `40d575d6` verified (a) F5 is a TWO-PART BUG not one — `topic_weights` NEVER computed anywhere in the codebase AND `source_weights` computed at `human_interface_service.py:732-736` but explicitly EXCLUDED from `update_learned_stats()` `.save(update_fields=[…])` scope at `models_human_interface.py:356-358`; the original "sets locally but never .save()'d" wording is imprecise for both parts; (b) Reader inventory: 1 real read of `source_weights` at `human_interface_service.py:717` with fallback 1.0 → silent identity multiplication given F1B; ZERO reads of `topic_weights` anywhere; ZERO frontend reads; 3 false-positive local dicts in `spider_priority_engine.py` + `recommendation_engine.py` + `ai_core/intelligence/orchestration.py`; (c) F5 severity at HEAD = MED silent no-op with UPGRADE-TO-HIGH pathway once fix ships (`_calculate_priority_score()` is downstream-consumed by HAI queue ordering per S1801 Cat A producer paths); (d) Signal chain BROKEN with two orphan cases (FeedbackProcessor bypass @ `models_feedback_processing.py:372-392` + auto-approve bypass @ `human_attention_lifecycle.py:279-325`); (e) **CROSS-SYSTEM PRIMITIVE STATUS: DISPROVEN** — grep of `user_pref_id\|user_preference_id\|preference_id` across all `.py` files at HEAD returns ZERO hits anywhere. `HumanPreference.id` (implicit PK) is NEVER referenced as identifier variable outside the model itself. Matches S1802 feedback_record_id + S1803 learning_event_id FAIL patterns; THIRD CONSECUTIVE HYPOTHESIS-DISPROVED-CROSS-SYSTEM outcome. **MC-3 CODIFICATION-READY promotion path DOES NOT advance at S1804 close** — running tally 1 pass / 3 disprove across 4 applications; meta-methodology finding candidate for xx99 §10 strengthens (primitive-box discipline earns its keep in TWO modes: pass = architectural spine verified; fail = domain-internal boundary verified). See S1804 §14 F1 + F5 + §20.7. | ~~P4 (Cat D) verifies F5 + reader inventory.~~ **VERIFIED-PARTIAL-AT-CHILD S1804 P4 Cat D** — F5 two-part bug root cause VERIFIED; reader inventory landed (1 read source_weights / 0 read topic_weights); severity MED silent no-op VERIFIED; signal chain gap two-orphan-case VERIFIED; cross-system-primitive DISPROVEN. |
| **`verification_id`** | S746 verification loop primitive on HAI schema. **HYPOTHESIS:** `verification_outcome` + `verified_at` + `verification_profit` + `event_completed_at` fields exist on HAI; but who WRITES them? What triggers `record_verification`? S1273 says trigger points undocumented. **HYPOTHESIS REMAINS at S1805 close (FOURTH CONSECUTIVE DISPROVED-CROSS-SYSTEM outcome after S1802 + S1803 + S1804):** S1805 P5 Cat E audit @ HEAD `3ff12391` verified (a) parent §5 D78 P5 wording DOUBLY OBSOLETE — S1503 sports arc already documented `betting_outcome_verifier.py:413` as automated trigger AND `verify_betting_outcomes` @ `core/tasks.py:6121-6184` has NO beat schedule entry per grep of `core/celery.py:1-1200` (Explore Agent 2 vs Agent 6 contradiction resolved by direct read of `core/celery.py:710-747`); (b) TWO real callers at HEAD (not zero): `views_human_interface.py:245` REST endpoint + `betting_outcome_verifier.py:413` unscheduled automated verifier; (c) verification chain SPORTS-ONLY (BettingOutcomeVerifier hard-filters `item_type='arbitrage'`); (d) REST endpoint verifications skip learning bridge (F8 fragile orchestrator-coupling); (e) STATUS_WATCHING has no expiry/stuck-item monitoring; (f) S1801 D5 durable-at-two — record_verification emits NO signal/event/log; (g) three-plane manual/automated/learning table shows only plane 1 LIVE (Rigby SIGN cycle 1 fold); (h) **CROSS-SYSTEM PRIMITIVE STATUS: DISPROVEN** — grep of `verification_id\|verification_pk\|verification_uuid\|verified_id` across all `.py` files at HEAD returns ONE hit at `intelligence/api/advisor_network_api.py:315` verified to be a DIFFERENT (advisor track record) verification object (not HAI). Matches S1802 feedback_record_id + S1803 learning_event_id + S1804 user_pref_id FAIL patterns; FOURTH CONSECUTIVE HYPOTHESIS-DISPROVED-CROSS-SYSTEM outcome. **MC-3 CODIFICATION-READY promotion path DOES NOT advance at S1805 close** — running tally 1 pass / 4 disprove across 5 applications; meta-methodology finding candidate for xx99 §10 CODIFICATION-CONFIRMED promotion with utility-rate-vs-pass-rate framing (utility rate 5/5 across 5 applications; pass rate 1/5 does not reflect discipline utility — see S1805 §20.7). See S1805 §14 F1 + F5 + §20.7. | ~~P5 (Cat E) verifies verification triggers + writer inventory + who-calls-record_verification.~~ **VERIFIED-PARTIAL-AT-CHILD S1805 P5 Cat E** — parent §5 D78 P5 wording drift VERIFIED; 2-caller inventory VERIFIED (REST + BettingOutcomeVerifier); automated path DEFINED-BUT-UNSCHEDULED VERIFIED (S1503 §1 Finding 1 durable-at-two); sports-only scope VERIFIED; learning-loop decoupling extended from S1801 D5 durable-at-two; three-plane partial-liveness picture landed; cross-system-primitive DISPROVEN. |
...
**Canonical entry points:**
- Model: `core/models_human_interface.py:230-265` (HumanFeedbackRecord). _[S1802 correction — was `:230-266`; :266 is blank between classes.]_
- Service: `core/services/human_interface_service.py:295-353` (`record_decision()`) + `:740-762` (`_feed_to_ml()`). _[S1802 correction — parent originally said `:738`; actual `_feed_to_ml` @ :740.]_
- Processor: `core/models_feedback_processing.py:33-330`
  (FeedbackProcessor CLASS) with `process_human_feedback` @ `:122`, `_reinforce_positive` @ `:216`, `_learn_from_negative` @ `:274`; inline classifier @ `:156-157` (no standalone `classify_positive_negative` method). _[S1802 correction — was `:122-180, 216-330`; those are method line-ranges, not class location.]_
- API: `POST /api/human/attention/<uuid:item_id>/decide/` @ `core/views_human_interface.py:546` (URL) + `:157-181` (view body). _[S1802 correction — was `/api/human/decisions/<id>/record/` which is INCORRECT at HEAD.]_
- Signal: `post_save` on HumanFeedbackRecord → FeedbackProcessor
  invocation @ `models_feedback_processing.py:372-392`.
