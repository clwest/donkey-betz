...
| Subsystem | Headline |
|---|---|
| [Agents](#agents) | 83 agents in AGENT_MAP (74 enabled, 8 rerouted, 1 blocked); 148 rows in Agent table. |
...
| [Celery Beat — Scheduled Tasks](#beat-schedule) | 45 enabled + 0 disabled = 45 PeriodicTask rows |
| [Personal Assistant (PA) Tools](#pa-tools) | 101 tool schemas + 166 registered handlers; 8 enrichment services |
| [Database Models](#database-models) | 574 concrete models across 23 apps |
| [URL Routes](#url-routes) | 1843 path() patterns across all core/urls*.py files |
| [Django View Files](#views-files) | 202 files matching core/views*.py |
...
| [Code Statistics](#code-stats) | 1,989 Python files, 977,913 lines across core/ + ai_core/ + intelligence/ |
...
**Headline:** 83 agents in AGENT_MAP (74 enabled, 8 rerouted, 1 blocked); 148 rows in Agent table.

**Code location:** `core/agent_router.py AGENT_MAP`

**Notes:** AGENT_MAP total = 83 (74 enabled + 8 rerouted + 1 blocked). DB Agent rows = 148. Top agent_type breakdown: income=20, content=17, career=15, business=14, job_search=12, finance=12, ai_ml=11, creative=11, marketing=10, analytics=7. Blocked: ['CodeGeneratorAgent']. Rerouted: ['AudioAgent', 'COOAgent', 'CTOAgent', 'CodeReviewAgent', 'DevOpsAgent', 'FullStackDeveloperAgent', 'VideoAgent', 'WorkflowAgent'].
...
**Headline:** 45 enabled + 0 disabled = 45 PeriodicTask rows
...
| ReplayBookmark | core |  |
| ... | _(174 more rows truncated)_ |

<a id="url-routes"></a>
## URL Routes

**Headline:** 1843 path() patterns across all core/urls*.py files
...
**Headline:** 202 files matching core/views*.py
...
**Headline:** 1,989 Python files, 977,913 lines across core/ + ai_core/ + intelligence/
