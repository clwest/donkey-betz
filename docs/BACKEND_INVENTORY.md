...
**Source of truth:** `docs/PLATFORM_INVENTORY.md` (regenerable). When this table disagrees with PLATFORM_INVENTORY, that wins.
...
<!-- @inventory-block:backend-summary -->
| Metric | Live Count |
|--------|------------|
| **Django Models** | 570 concrete models across 23 apps |
| **URL Endpoints** | 1841 path() patterns across all core/urls*.py files |
...
| **Views Files** | 200 files matching core/views*.py |
| **Management Commands** | 154 management commands in core/management/commands/ |
| **Discord Commands** | 96 @*.command decorators, 48 @app_commands.command, 25 Cog classes in discord_bot.py |

_Auto-generated by `refresh_doc_inventory_blocks` from `gather_inventory()`. Do not hand-edit between markers._
<!-- @inventory-block:end -->

> The hand-maintained "Manual Snapshot" table that previously lived
> here was removed during Session 1100 doc-drift cleanup. Numbers
> drifted across Jan/Apr 2026 and competed with the auto-block above.
> For the canonical, regenerable snapshot of every count (models,
> URLs, tasks, agents, spiders, services, views, management commands,
> WebSocket consumers, Discord commands, body systems, LLM providers,
> code statistics, etc.), see
> [`docs/PLATFORM_INVENTORY.md`](/docs/PLATFORM_INVENTORY.md) — the
> runtime anchor. Regenerate with
