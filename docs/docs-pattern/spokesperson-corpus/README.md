...
This pattern is a candidate for `context-kit spokesperson` subcommand. While it lives in
project-local `docs/docs-pattern/` as a working spec; once the shape stabilizes through
one or two real instances, it should port into context-kit proper so any context-kit
adopter can scaffold a corpus with `context-kit spokesperson init`.
