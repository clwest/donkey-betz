...
# names are unchanged (locked by Phase 1 name= kwargs), so 25 PeriodicTask
# DB rows (24 enabled, 1 disabled — run-stock-financial-agents — pre-existing
# operator state, unchanged by this PR), settings.py task-routing dicts
# including the 2 non-standard names — learning_loop.track_prediction_outcomes,
# autonomous.blockchain_security_monitor — and 3 add_critical_celery_tasks
# string dispatchers all continue to resolve. core/tasks.py keeps a re-export
# shim (despite zero current Python-import consumers — preserves the interface
# in case future callers add them).
#
# `_run_agent_group` is imported from core.tasks above (added to the existing
# helper-import block). With this PR, the LAST two out-of-scope callers
# (run_stock_financial_agents, run_prediction_market_agents) move into this
# module, leaving zero callers in core/tasks.py — see TODO at the helper's
# definition for the planned move to a shared runtime module.
