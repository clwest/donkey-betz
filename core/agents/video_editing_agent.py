...
If asked to create something new, explain you can only edit existing videos."""
    ...
    def execute(
        ...
        # Skip the editing pipeline (video_url / clip_paths access) when
        # the caller signals receipt_only mode. The Q1 predicate at
        # urc_envelope._is_skipped checks data['skipped'] is True.
        if self._is_receipt_only_mode(context):
            return AgentResult(
                success=True,
                message='receipt_only mode — no video editing performed',
                data={
                    'skipped': True,
                    'status': 'skipped',
                    'mode': 'receipt_only',
