...
@require_http_methods(["GET"])
def initiative_hub_api(request):
    """
    Session 899: Unified Initiative Hub API.

    Returns ALL initiatives with their full origin traces, plus orphaned
    conversations and dreams that haven't yet created initiatives.

    This is the single source of truth for understanding system activity.
    Everything flows: Trigger → Conversation → Decision → Initiative → Stages → Deliverable
    """
    try:
        from core.models_document_registry import Initiative, InitiativeStage, STAGE_NAMES
        from core.models_unified_system import AgentConversation, AgentDream, AgentDecisionSummary
        from core.models import Deliverable
        from django.db.models import Prefetch, Q, Count
        from django.utils import timezone
        from datetime import timedelta

        # Time window for orphaned items (last 7 days)
        cutoff = timezone.now() - timedelta(days=7)

        # ===== 1. GET ALL INITIATIVES WITH FULL TRACES =====
        initiatives_data = []

        # Prefetch all related data to avoid N+1 queries
        initiatives = Initiative.objects.prefetch_related(
            'source_decisions',
            'source_decisions__conversation',
            'source_decisions__conversation__participants',
            'source_decisions__hive_session',
            Prefetch('stages', queryset=InitiativeStage.objects.order_by('stage')),
        ).order_by('-updated_at')[:50]  # Limit to 50 most recent

        for initiative in initiatives:
            # Build trace inline (similar to initiative_origin_trace_api but batched)
            trace = {
                'id': str(initiative.id),
                'name': initiative.name,
                'description': initiative.description,
            ...
            # Get source decision
            decisions = list(initiative.source_decisions.all())
            if decisions:
                ...
                if decision.conversation:
                    ...
                    trace['conversation'] = {
                        ...
                        'message_count': conv.message_count,
                        'quality_score': conv.quality_score,
                        'conclusion': conv.conclusion[:300] if conv.conclusion else None,
                        'trigger_type': conv.trigger_type,
                        'status': conv.status,
                        'started_at': conv.started_at.isoformat() if conv.started_at else None,
                    }
                    trace['trigger'] = {
                        'type': conv.trigger_type or 'unknown',
                        'description': f'{conv.trigger_type} triggered' if conv.trigger_type else 'Unknown trigger',
                    }
                    # Get agent names
                    ...
                    trace['conversation'] = {
                        ...
                        'topic': hive.conversation_topic or (hive.question[:100] if hive.question else None),
                        'contribution_count': hive.contribution_count,
                        'synthesis_summary': hive.synthesis_summary[:300] if hive.synthesis_summary else None,
                        'status': hive.status,
                        'started_at': hive.started_at.isoformat() if hive.started_at else None,
                    }
                    trace['trigger'] = {
                        'type': 'scheduled' if hive.auto_selected_agents else 'manual',
                        'description': 'Scheduled discussion' if hive.auto_selected_agents else 'Manual conversation',
                    }
            ...
            # Get deliverable (separate query, but should be fast)
            ...
            trace['completeness_score'] = sum([
                20 if trace['conversation'] else 0,
                20 if trace['decision'] else 0,
                20 if trace['agents'] else 0,
                20 if trace['stages'] else 0,
                20 if trace['deliverable'] else 0,
            ])

            initiatives_data.append(trace)

        # ===== 2. GET ORPHANED CONVERSATIONS =====
        # Conversations that don't have a decision linked to an initiative
        orphaned_conversations = []

        # Get decision IDs that ARE linked to initiatives
        linked_decision_ids = AgentDecisionSummary.objects.filter(
            initiative__isnull=False
        ).values_list('conversation_id', flat=True)

        # Get recent conversations NOT linked through decisions
        unlinked_convos = AgentConversation.objects.filter(
            started_at__gte=cutoff,
            message_count__gt=0,  # Has actual content
        ).exclude(
            id__in=linked_decision_ids
        ).select_related().order_by('-started_at')[:20]

        for conv in unlinked_convos:
            participants = list(conv.participants.values_list('name', flat=True)[:3])
            orphaned_conversations.append({
                'id': str(conv.id),
                'type': 'conversation',
                'topic': conv.topic,
                'agents': participants,
                ...
                'status': conv.status,
                'trigger_type': conv.trigger_type,
                'conclusion': conv.conclusion[:200] if conv.conclusion else None,
                'started_at': conv.started_at.isoformat() if conv.started_at else None,
                'quality_score': conv.quality_score,
            })

        # ===== 3. GET ORPHANED DREAMS =====
        orphaned_dreams = []
        ...
        for dream in dreams:
            orphaned_dreams.append({
                'id': str(dream.id),
                'type': 'dream',
                'title': dream.title[:100] if dream.title else 'Untitled',
                'agent': dream.agent.name if dream.agent else 'Unknown',
                'content_preview': dream.content[:200] if dream.content else None,
                'dream_type': getattr(dream, 'dream_type', None),
        ...
        stats = {
            ...
            'active_initiatives': sum(1 for i in initiatives_data if i['status'] == 'ACTIVE'),
            'completed_initiatives': sum(1 for i in initiatives_data if i['status'] == 'COMPLETED'),
            'orphaned_conversations': len(orphaned_conversations),
            'orphaned_dreams': len(orphaned_dreams),
            'avg_completeness': sum(i['completeness_score'] for i in initiatives_data) / max(len(initiatives_data), 1),
        }
        ...
        return JsonResponse({
            ...
            'orphaned': {
                ...
                'dreams': orphaned_dreams,
            },
            'timestamp': timezone.now().isoformat(),
        })

    except Exception as e:
        logger.error(f"Error in initiative_hub_api: {e}", exc_info=True)
        return JsonResponse({'success': False, 'error': str(e)}, status=500)
