...
# Phase 3 re-exports — diagnostic tasks now live in core/tasks_diagnostics.py.
# Imports preserved here so existing callers (`from core.tasks import X`,
# `core.tasks.X.apply_async()`, and `getattr(import_module("core.tasks"), "X")`
# inside `scheduled_diagnostic_runner`) keep working without modification.
# The tasks themselves register under the same Celery names regardless of
# which module defines them.
from core.tasks_diagnostics import (  # noqa: F401
    run_cto_daily_diagnostic,
    post_cto_daily_diagnostic,
    run_coo_daily_diagnostic,
    post_coo_daily_diagnostic,
    run_trend_daily_diagnostic,
    post_trend_daily_diagnostic,
...
# Phase 3 re-exports — experiment + KPI tasks now live in core/tasks_experiments.py.
# Imports preserved here so existing callers (`from core.tasks import X`,
# `core.tasks.X.apply_async()`, settings.py task-routing dicts keyed by
# 'core.tasks.X', PeriodicTask DB rows, and the management-cmd
# `add_critical_celery_tasks` string dispatcher) keep working without any
# modification. Tasks register under the same Celery names regardless of
# which module defines them.
#
# The private helper `_send_kpi_update_discord_notification` is NOT
# re-exported — it has no callers outside `update_experiment_kpis` and
# moves with that task.
from core.tasks_experiments import (  # noqa: F401
...
# Phase 3 re-exports — learning tasks now live in core/tasks_learning.py.
# Imports preserved here so existing callers (`from core.tasks import X`,
# settings.py task-routing dicts keyed by 'core.tasks.X' (and
# 'core.check_learning_loop_slo' — non-standard), PeriodicTask DB rows,
# the `add_critical_celery_tasks` management-cmd string dispatcher,
# `bootstrap_learning_system`, `views_projects_api`,
# `warmup_body_systems`) keep working without any modification. Tasks
# register under the same Celery names regardless of which module
# defines them — `check_learning_loop_slo` is registered as
# `core.check_learning_loop_slo` and survives the move verbatim.
from core.tasks_learning import (  # noqa: F401
...
# Phase 3 re-exports — ops workflows / triggers / human-attention tasks now live in core/tasks_ops.py.
...
# ROI, ML, audit, desks, health, cleanup, agents, content, and financial
# re-exports below.
#
# Re-exported here so 8 PeriodicTask DB rows (all enabled — no disabled
# state), 10 settings.py task-routing entries (8 standard `core.tasks.*`
# + 2 non-standard `triggers.*` + 1 pre-existing duplicate for
# `generate_smart_suggestions`), and 4 lazy Python-import consumer
# sites across 4 modules (signals.trigger_signals, tasks_misc,
# views_workflow_run, services.td_handlers_core) keep resolving.
#
# `execute_scheduled_workflow.delay(...)` is dispatched from
# tasks_misc.py:603 (lazy `from core.tasks import execute_scheduled_workflow`).
# The shim preserves the import path and Phase 1 locked the registered
# name, so the cross-sibling dispatch survives with zero changes to
# tasks_misc.
from core.tasks_ops import (  # noqa: F401
    # Workflows & automations
    execute_scheduled_workflow,
    sync_workflow_schedules,
    check_workflow_schedules,
    execute_scheduled_automations,
    run_source_pack_workflow,
    # Smart suggestions
    ...
    process_hitl_escalations,
    scan_concerns_for_human_action,
    generate_human_attention_items,
    process_human_attention_lifecycle,
)



# Phase 3 re-exports — ops ROI / analytics / opportunity tasks now live in core/tasks_ops.py.
# IMPORTANT: this block MUST live at the bottom of the file. tasks_ops.py
# imports private helpers from core.tasks at module top, so triggering
# `from core.tasks_ops import …` before those helpers are defined produces
# a partially-initialized-module ImportError. Same constraint as the ops
# ML, ops audit, ops desks, ops health, ops cleanup, agents, content, and
# financial re-exports below.
#
# Re-exported here so 11 PeriodicTask DB rows (10 enabled + 1 disabled —
# proactive-opportunity-alerts pre-existing operator state, unchanged),
# 17 settings.py task-routing entries (including the 4 non-standard
# `roi_metrics.*` registered names + 3 pre-existing duplicate entries),
# and 6 lazy Python-import consumer sites across 2 view modules
# (views_auto_distribution ×2, views_opportunity ×4) keep resolving.
#
...
# name). Phase 1 locked those names, so the dispatch survives this
# module relocation with zero changes to tasks_learning.
from core.tasks_ops import (  # noqa: F401
    # Opportunity lifecycle
    execute_pending_opportunity_tasks,
    generate_opportunity_report,
    generate_weekly_opportunity_digest,
    send_proactive_opportunity_alerts,
    send_personalized_opportunity_alerts,
    process_high_scoring_opportunities,
    # Distribution
    ...
    record_opportunity_click,
    record_opportunity_application,
...
# Phase 3 re-exports — ops cleanup & retention tasks now live in core/tasks_ops.py.
# IMPORTANT: this block MUST live at the bottom of the file. tasks_ops.py
# imports private helpers from core.tasks at module top, so triggering
# `from core.tasks_ops import …` before those helpers are defined produces
# a partially-initialized-module ImportError. Same constraint as the agents,
# content, and financial re-exports below.
#
# Re-exported here so 13 beat schedule entries (core/celery.py — incl. the
# non-standard `cleanup_expired_signals`), 11 PeriodicTask DB rows, 6
# settings.py task-routing entries, and the `add_critical_celery_tasks`
# management-cmd dispatcher keep resolving. No `from core.tasks import
# <name>` consumer sites exist for these 19 tasks; the re-export shim is
# preserved defensively. Subsequent ops sub-PRs (2 through 9) will land
# additional re-export blocks above this one.
from core.tasks_ops import (  # noqa: F401
    # Cleanup tasks
    reap_zombie_work,
    ...
    cleanup_stale_dreams,
    cleanup_celery_task_events,
    cleanup_llm_call_logs,
    cleanup_expired_uploads,
    cleanup_old_resolve_jobs,
    cleanup_audio_cache,
    # Retention enforcement
    cleanup_resolved_signatures,
    auto_archive_stale_deliverables,
    enforce_db_retention,
    cleanup_expired_pa_insights,
    enforce_data_retention,
    # Validation expiry
    cleanup_stale_scoring_requests,
    expire_overdue_validations,
    cleanup_expired_signals,
    check_orphan_deliverables,
)
