...
"""
            ...
            result = agent.execute(prompt=prompt, context={
                'pilot_id': str(implementation.pilot.id),
                'decision_id': str(decision.id),
                'feature_spec': feature_spec
            })

            # Create a task for human review of the generated code
            from core.models_unified_system import ActionableTask
            task = ActionableTask.objects.create(
                title=f"Review: {decision.topic[:80]}",
                description=f"Code generated by FullStackDeveloperAgent for pilot implementation.\n\n{result.get('response', '')}",
                task_type='implementation_review',
                priority='medium',
                metadata={
                    'pilot_id': str(implementation.pilot.id),
                    'implementation_id': str(implementation.id),
                    'generated_by': 'FullStackDeveloperAgent'
                }
            )
            ...
            return {
                ...
                'summary': f"Code generated and task created for human review: {task.id}"
            }
