...
class OpsHandlersMixin:
    ...
    def _handle_ops(self, tool_name: str, payload: Dict[str, Any], user_id: Optional[int], trace_id: str) -> Dict[str, Any]:
        ...
        elif action == 'zoom_out_ledger':
            # S2779 N22 v2: PA-tool read surface for the Rigby SIGN
            # zoom-out concern ledger at logs/zoom_out_classifications.jsonl.
            # Promotes the CLI-only zoom_out_streak_report to a Rigby-
            # consumable surface so SIGN loops can consult prior folds.
            # Advisory-only by explicit design (is_gate: false in response
            # body) per S2777 tail-wags-dog fold. Constitutional context:
            # PLAYBOOK-6.10.8 mandates persistence via record_zoom_out_concern;
            # this action mirrors the same ledger back as read surface.
            return self._ops_zoom_out_ledger(payload, trace_id)
    ...
    def _ops_zoom_out_ledger(
        self, payload: Dict[str, Any], trace_id: str
    ) -> Dict[str, Any]:
        """S2779 N22 v2: read logs/zoom_out_classifications.jsonl.

        Promotes the S2777 CLI-only read surface (zoom_out_streak_report)
        into a PA-tool action. The future consumer is Rigby herself during
        joint SIGN loops — 'show me the last N zoom-out folds from arc X
        so I don't repeat myself.' Advisory-only by explicit design: the
        response embeds ``advisory`` header + ``is_gate: false`` +
        ``semantics: "advisory_pattern_evidence"`` to keep the "pattern
        evidence, not gate" stance intact per S2777 tail-wags-dog fold and
        S2779 T1 SIGN V4 fold.
        ...
        always present in the response (even when zero) per S2779 T1 SIGN
        V2 fold — schema-consistency over hide-when-clean.
        ...
        if session_raw is not None:
            try:
                session_filter = int(session_raw)
            except (TypeError, ValueError):
                session_filter = None
