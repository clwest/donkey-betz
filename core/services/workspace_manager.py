...
class FileWriter:
    ...
    def _resolve_workspace_path(
        ...
        # Session 1246 (S1247 lane G): use env-aware path translator so
        # workspaces created on Railway resolve to local dev paths when
        # WORKSPACE_BASE_DIR is set. Falls back to stored root_path.
        root = Path(workspace.effective_root_path).resolve(strict=False)
    ...
    def status(self, workspace: ProjectWorkspace) -> Dict[str, Any]:
        ...
        try:
            result = subprocess.run(
                ...
                cwd=workspace.effective_root_path,
                capture_output=True,
                text=True,
                timeout=30
            )
