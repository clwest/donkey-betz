...
def update_assistant_configuration():
    ...
    update_script = '''# Instructions to Apply RAG Integration to Assistant

## Quick Fix (Recommended)

Add this to your `core/views.py` file, replacing the existing `assistant_chat` function:

```python
from django.db.models import Q

def search_embeddings_simple(query, user, limit=5):
...
# Then modify the assistant_chat function to include this before calling the AI:

# Get relevant context from embeddings
relevant_docs = search_embeddings_simple(message, user, limit=3)
context_info = ""
if relevant_docs:
    context_info = "\\n\\nRelevant information from your knowledge base:\\n"
    for doc in relevant_docs:
        context_info += f"- {doc['title']}: {doc['content']}...\\n"

# Add context_info to the system_prompt before the AI call
```

## Full Integration (Advanced)

1. Copy the enhanced assistant from `core/views_assistant_rag_enhanced.py`
2. Update `core/urls.py` to use the enhanced version
...
- Search through your 265,318 embeddings
- Provide context-aware responses
- Not default to sports betting topics
'''
...
📝 Next Steps:
   1. Apply the RAG integration:
      - Check APPLY_RAG_TO_ASSISTANT.md for instructions
      - Update core/views.py with the RAG search function
