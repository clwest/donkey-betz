"""
Session 128: Test apply_color_grading GPT function calling

Tests that natural language commands like:
"Make video 7 look cinematic"

Successfully trigger the apply_color_grading tool.
"""
import os
import sys
import django
...
print("=" * 80)
print("🧪 SESSION 128 - COLOR GRADING TEST")
print("=" * 80)

# Step 1: Get a video to color grade
...
# Step 2: Test color grading via AI Assistant
print(f"\n🎨 STEP 2: Testing color grading via AI Assistant...")
assistant = EnhancedPersonalAIAssistant(user=user)

# Test command: Apply cinematic color grading
test_message = f"Make video {video_seq_num} look cinematic"
...
has_success = '✅' in response_text and 'color grading' in response_text.lower()

print(f"   Response preview: {response_text[:200]}...")
print(f"   Success: {has_success}")

if has_success:
    # Check if a new video was created with color grading
    ...
    if new_videos:
        ...
        print("   The AI successfully applied color grading using GPT function calling!")
else:
    print(f"\n⚠️  COLOR GRADING TEST FAILED")
