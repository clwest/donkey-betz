...
**Last Updated:** July 12, 2026 (Session 2772 — Ops-endpoint auth-regression suite + staff gate (N16). Every `/api/ops/*` endpoint now requires `is_staff` alongside `@login_required`; module docstring codifies a 6-rule ops-endpoint scope policy; 15 Django tests lock the contract (6 anon-blocked + 6 non-staff-blocked + 1 staff-happy + 2 route-inventory guards). Rigby's Q3 open-ended zoom-out (S2771 rule applied for first time) surfaced 7 substantive concerns; 3 shipped same-PR, 4 forward-carry with explicit triggers. Seventh close-cycle post-codification. L7 constitutional anchor unchanged; Playbook v0.6.0 remains latest ratified version.)
