...
**Last Updated:** July 11, 2026 (Session 2770 — health_summary PARTIAL_RECYCLE verdict (N11). `health_summary` composition gains a fifth backend call to `recent_recycles` limit=1; verdict overrides to `PARTIAL_RECYCLE` when base is `STALE_*` AND newest recycle event has `partial_recycle=true` (N7 evidence); new optional `partial_recycle_details` field carries surviving_processes + recycle_sha_short. Frontend verdict enum extended additively; amber tile color; warning row lists surviving processes and prompts `make recycle-all`. Closes the S2768 N7 evidence-to-alert loop. Fifth close-cycle post-codification. L7 constitutional anchor unchanged; Playbook v0.6.0 remains latest ratified version.)
