...
**Last Updated:** July 12, 2026 (Session 2773 — Ops query-param allowlist + `_call_ops_tool` helper refactor + `_parse_date_param` calendar-validation fix (N18v2 — scope-expanded from N18 per Chris D-verdict on Rigby zoom-out). Every `/api/ops/*` endpoint now rejects unknown query params with 400 + machine-stable `code='unknown_query_params'` body; 4-copy `_Proxy(OpsHandlersMixin)` boilerplate extracted to one helper; `_parse_date_param` uses `datetime.date.fromisoformat` (previously accepted `2026-99-99` as valid). New `test_ops_query_param_allowlist_2773.py` — 22 tests, combined with S2772 suite = 37/37 PASS. Rigby's zoom-out (S2771 rule, third consecutive application) surfaced 5 substantive concerns; 3 shipped same-PR, 2 forward-carry with explicit triggers. Eighth close-cycle post-codification. L7 constitutional anchor unchanged; Playbook v0.6.0 remains latest ratified version.)
