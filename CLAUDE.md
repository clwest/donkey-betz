...
**Last Updated:** July 11, 2026 (Session 2769 — CCL v2 search + filter (N8). `close_ceremony_ledger` view extended with `session_min / session_max / envelope_only / date_from / date_to` params + `total_available` field; frontend gains a compact filter row above Recent Close-Ceremonies plus a "Show up to 50" expand button. Opportunistic fix: `_DATE_LINE_RE` broadened to accept both `**Date:**` markdown (older) and YAML `date:` frontmatter (S2767+) — previously silent-null. Fourth close-cycle post-codification. L7 constitutional anchor unchanged; Playbook v0.6.0 remains latest ratified version.)
