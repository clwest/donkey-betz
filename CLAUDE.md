...
**Last Updated:** July 11, 2026 (Session 2768 — Recycle-all emitter enriched with worker PIDs before/after (N7). New `scripts/emit_recycle_event.py` stdlib helper, Makefile `recycle-all` sequentialized to snapshot → restart → emit, `_ops_recent_recycles` handler pass-through of new fields. Every `make recycle-all` now writes `pids_before / pids_after / partial_recycle / surviving_processes` — partial recycles become machine-observable in the same log PLAYBOOK-7.4.4 uses as evidence. Third close-cycle post-codification. L7 constitutional anchor unchanged; Playbook v0.6.0 remains latest ratified version.)
