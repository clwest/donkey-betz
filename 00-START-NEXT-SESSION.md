...
**No in-flight arc.** S3026 → S3035 canonical-lifecycle arc closed on both sides (promotion + rejection) AND now has UI visibility. Two operator carries still open:
- **S3030 prod deploy carry** — run `python manage.py backfill_canonical_drift --apply` against Railway prod when convenient.
- **S3035 manual smoke** — after next `make start`, open BoardroomTab, promote or reject a draft decision, verify the panel updates within 10s and counters increment.
